...
class TrainedModel(object):
  ...
  #NEWLY ADDED!!!
  def check_viability(self, note_sequences, assert_same_length=False):
    if not self._config.hparams.conditional:
      raise RuntimeError('Cannot encode with a non-conditional model.')

    j = 0
    relevant_bits = []
    for i, note_sequence in enumerate(note_sequences):
      extracted_tensors = self._config.data_converter.to_tensors(note_sequence)
      if not extracted_tensors.inputs:
        continue
      elif len(extracted_tensors.inputs) > 1:
        # print('Found multiple examples, using first for  %s' % note_sequence.filename)
        # extracted_tensors.inputs = extracted_tensors.inputs[0]
        relevant_bits.append(note_sequence.filename)
        continue
      else:
        # print('YES EXAMPLES FOUND for  %s' % note_sequence.filename)
        pass
      relevant_bits.append(note_sequence.filename)
      if j == 100:
        j = 0
        print("handled " + str(i) + " notesequences.")
      else:
        j+=1
  ...
  def encode(self, note_sequences, assert_same_length=False):
    ...
    for note_sequence in note_sequences:
      extracted_tensors = self._config.data_converter.to_tensors(note_sequence)
      if not extracted_tensors.inputs:
        # print('NO EXAMPLES FOUND for  %s' % note_sequence.filename)
        continue
      ...
      if len(extracted_tensors.inputs) > 1:
        ...
        # print('Multiple (%d) examples extracted from NoteSequence: %s' %(len(extracted_tensors.inputs), note_sequence.filename))
        # continue
        pass
        # relevant_bits.append(note_sequence.filename)
        # print('Found multiple examples, using first for  %s' % note_sequence.filename)
    ...
    return relevant_bits, self.encode_tensors(inputs, lengths, controls)
